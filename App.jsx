...
function BackupModal({ data, onRestore, onClose }) {
  ...
  const handleExport = () => {
    // Try native share first (iOS Safari)
    ...
    setMsg({type:"ok", text:"バックアップファイルを書き出しました！"});
  };

  const handleCopyJson = () => {
    navigator.clipboard.writeText(jsonStr).then(()=>setMsg({type:"ok",text:"JSONをクリップボードにコピーしました！"})).catch(()=>setMsg({type:"err",text:"コピーに失敗しました"}));
  };
  ...
  const handleImport = () => {
    try {
      ...
      const required = ["categories","logs","diaries","goalHours"];
      for(const k of required){ if(!(k in parsed)) throw new Error(`キー "${k}" がありません`); }
      onRestore(parsed);
      setMsg({type:"ok", text:"データを復元しました！"});
      setTimeout(onClose, 1200);
    } catch(e) {
      setMsg({type:"err", text:"形式エラー: " + e.message});
    }
  };
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:400}} onClick={onClose}>
      <div style={{background:"#1e2330",borderRadius:"20px 20px 0 0",border:"1px solid #2a2f3d",padding:24,width:"100%",maxWidth:480,paddingBottom:36}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
          <div>
            <div style={{fontSize:16,fontWeight:800}}>💾 バックアップ</div>
            <div style={{fontSize:10,color:"#6b7a99",marginTop:2}}>データサイズ: {dataSize} KB</div>
          </div>
          <button onClick={onClose} style={{background:"none",border:"none",color:"#6b7a99",fontSize:20,cursor:"pointer",lineHeight:1}}>×</button>
        </div>

        {/* Stats */}
        <div style={{display:"flex",gap:8,marginBottom:20}}>
          {[
            {label:"記録", value:data.logs?.length||0, unit:"件", color:"#4f9eff"},
            {label:"日記", value:Object.keys(data.diaries||{}).filter(k=>data.diaries[k]?.trim()).length, unit:"件", color:"#fbbf24"},
            {label:"カテゴリー", value:data.categories?.length||0, unit:"個", color:"#34d399"},
          ].map(s=>(
            <div key={s.label} style={{flex:1,background:"#161920",borderRadius:10,padding:"10px 8px",textAlign:"center",border:"1px solid #2a2f3d"}}>
              <div style={{fontSize:18,fontWeight:800,color:s.color}}>{s.value}<span style={{fontSize:11}}>{s.unit}</span></div>
        ...
        {msg&&(
          <div style={{background:msg.type==="ok"?"rgba(52,211,153,0.12)":"rgba(248,113,113,0.12)",border:`1px solid ${msg.type==="ok"?"#34d399":"#f87171"}`,borderRadius:8,padding:"10px 14px",marginBottom:14,fontSize:12,color:msg.type==="ok"?"#34d399":"#f87171",fontWeight:600,textAlign:"center"}}>
        ...
        {mode==="menu"&&(
          <>
            ...
            <button style={bS("#2a2f3d")} onClick={()=>setMode("import")}>📥 バックアップから復元する</button>
          </>
        )}

        {mode==="export"&&(
          <>
            <div style={{fontSize:12,color:"#6b7a99",marginBottom:12}}>書き出し方法を選んでください</div>
            <button style={bS("#4f9eff")} onClick={handleExport}>📄 JSONファイルとして保存</button>
            <button style={bS("#2a2f3d")} onClick={handleCopyJson}>📋 JSONをクリップボードにコピー</button>
            <div style={{background:"#161920",borderRadius:8,padding:10,marginBottom:10,border:"1px solid #2a2f3d",maxHeight:120,overflowY:"auto"}}>
              <pre style={{fontSize:9,color:"#6b7a99",margin:0,whiteSpace:"pre-wrap",wordBreak:"break-all"}}>{jsonStr.slice(0,300)}...</pre>
            </div>
        ...
        {mode==="import"&&(
          <>
            <div style={{fontSize:12,color:"#6b7a99",marginBottom:8}}>バックアップのJSONを貼り付けてください</div>
            <textarea value={importText} onChange={e=>setImportText(e.target.value)}
              placeholder='{"categories":[...],"logs":[...],...}'
              style={{width:"100%",boxSizing:"border-box",height:140,background:"#161920",border:"1px solid #2a2f3d",borderRadius:10,padding:12,color:"#e8ecf4",fontSize:11,resize:"none",outline:"none",fontFamily:"monospace",marginBottom:10}}/>
            <div style={{background:"rgba(248,113,113,0.08)",border:"1px solid rgba(248,113,113,0.3)",borderRadius:8,padding:"8px 12px",marginBottom:12,fontSize:11,color:"#f87171"}}>
              ⚠ 復元すると現在のデータが上書きされます
            </div>
            <button style={bS("#f87171")} onClick={handleImport}>復元する</button>
            <button style={{...bS("#161920","#6b7a99"),border:"1px solid #2a2f3d"}} onClick={()=>setMode("menu")}>← 戻る</button>
          </>
        )}
      </div>
...
function useTouchDrag(onDrop) {
  ...
  const createGhost = (label, x, y) => {
    const el = document.createElement("div");
    el.textContent = "⠿ " + label;
    Object.assign(el.style, {
      position:"fixed", left:`${x-70}px`, top:`${y-20}px`,
      background:"#2a2f3d", color:"#e8ecf4", borderRadius:"10px",
      padding:"8px 16px", fontSize:"13px", fontWeight:"700",
      pointerEvents:"none", zIndex:"9999", opacity:"0.96",
      boxShadow:"0 8px 32px rgba(0,0,0,0.7)", border:"1.5px solid #4f9eff",
  ...
  const removeGhost = () => {
    if(ghost.current){ ghost.current.style.opacity="0"; setTimeout(()=>{ ghost.current?.remove(); ghost.current=null; },150); }
  };

  const getDropZone = (x, y) => {
    for(const z of document.querySelectorAll("[data-dropzone]")){
      const r=z.getBoundingClientRect();
      if(x>=r.left&&x<=r.right&&y>=r.top&&y<=r.bottom) return Number(z.dataset.dropzone);
    }
    return null;
  };

  const clearHighlights = () => {
    document.querySelectorAll("[data-dropzone]").forEach(z=>{ z.style.outline=""; z.style.background=""; });
  };

  const getDragProps = useCallback((task, fromDay) => ({
    onTouchStart: e => {
      const touch = e.touches[0];
      longTimer.current = setTimeout(() => {
        dragging.current = { task, fromDay };
        createGhost(task.label, touch.clientX, touch.clientY);
        if(navigator.vibrate) navigator.vibrate(50);
        document.querySelectorAll("[data-dropzone]").forEach(z=>{
          if(Number(z.dataset.dropzone)!==fromDay) z.style.outline="2px dashed rgba(79,158,255,0.5)";
        });
      }, 500);
    },
    onTouchMove: e => {
      // Cancel long-press if finger moves before 500ms
      if(!dragging.current){ clearTimeout(longTimer.current); return; }
      e.preventDefault();
      const touch = e.touches[0];
      if(ghost.current){ ghost.current.style.left=`${touch.clientX-70}px`; ghost.current.style.top=`${touch.clientY-20}px`; }
      const dz = getDropZone(touch.clientX, touch.clientY);
      dropZone.current = dz;
      document.querySelectorAll("[data-dropzone]").forEach(z=>{
        const idx=Number(z.dataset.dropzone);
        z.style.outline = idx===dz ? "2px solid #4f9eff" : (idx!==fromDay?"2px dashed rgba(79,158,255,0.3)":"");
        z.style.background = idx===dz ? "rgba(79,158,255,0.1)" : "";
      });
    },
    onTouchEnd: () => {
      ...
      if(dz !== null && dz !== dragging.current.fromDay){
        onDrop(dragging.current.task, dragging.current.fromDay, dz);
        if(navigator.vibrate) navigator.vibrate([20,10,20]);
      }
      dragging.current = null; dropZone.current = null;
    },
    onTouchCancel: () => {
      clearTimeout(longTimer.current);
      clearHighlights(); removeGhost();
      dragging.current = null; dropZone.current = null;
    },
    // PC: mouse drag
    draggable: true,
    onDragStart: () => { dragging.current = { task, fromDay }; },
    onDragEnd: () => { clearHighlights(); dragging.current = null; },
  }),[onDrop]);

  const getDropZoneProps = useCallback((dayIdx) => ({
    "data-dropzone": dayIdx,
    onDragOver: e => { e.preventDefault(); },
    onDrop: () => {
      if(!dragging.current || dragging.current.fromDay===dayIdx) return;
      onDrop(dragging.current.task, dragging.current.fromDay, dayIdx);
      clearHighlights(); dragging.current = null;
    },
  }),[onDrop]);

  return { getDragProps, getDropZoneProps };
}
  ...
  return (
    <div style={{marginBottom:10}}>
      ...
      <div style={{height:12,borderRadius:6,background:"#161920",overflow:"hidden",display:"flex",border:"1px solid #2a2f3d"}}>
        {segs.map((s,i)=><div key={i} title={`${s.name}: ${fmtHM(s.dur)}`} style={{width:`${(s.dur/86400)*100}%`,background:s.color,minWidth:s.dur>60?2:0,transition:"width 0.5s"}}/>)}
      </div>
      {segs.length>0&&(
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:300}} onClick={onClose}>
      <div style={{background:"#1e2330",borderRadius:"20px 20px 0 0",border:"1px solid #2a2f3d",padding:24,width:"100%",maxWidth:480,paddingBottom:36}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <div><div style={{fontSize:15,fontWeight:800}}>📔 日記</div><div style={{fontSize:11,color:"#fbbf24"}}>{date}</div></div>
        ...
        <textarea value={text} onChange={e=>setText(e.target.value)} placeholder="今日のことを書いておこう..."
          style={{width:"100%",boxSizing:"border-box",height:220,background:"#161920",border:"1px solid #2a2f3d",borderRadius:10,padding:14,color:"#e8ecf4",fontSize:14,resize:"none",outline:"none",lineHeight:1.8,fontFamily:"inherit"}} autoFocus/>
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:300}} onClick={onClose}>
      <div style={{background:"#1e2330",borderRadius:"20px 20px 0 0",border:"1px solid #2a2f3d",padding:20,width:"100%",maxWidth:480,maxHeight:"85vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="日付・内容で検索..."
          style={{background:"#161920",border:"1px solid #2a2f3d",borderRadius:8,padding:"7px 12px",color:"#e8ecf4",fontSize:12,outline:"none",marginBottom:10,width:"100%",boxSizing:"border-box"}}/>
        <div style={{overflowY:"auto",flex:1}}>
          ...
          {filtered.map(([date,text])=>(
            <div key={date} onClick={()=>onOpen(date)} style={{background:"#161920",borderRadius:10,border:"1px solid #2a2f3d",padding:"12px 14px",marginBottom:8,cursor:"pointer"}}>
              ...
              <div style={{fontSize:12,color:"#94a3b8",lineHeight:1.6,overflow:"hidden",display:"-webkit-box",WebkitLineClamp:3,WebkitBoxOrient:"vertical",whiteSpace:"pre-wrap"}}>{text}</div>
            </div>
          ))}
        </div>
        <button onClick={onClose} style={{background:"none",border:"none",color:"#6b7a99",cursor:"pointer",fontSize:12,paddingTop:12}}>閉じる</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{background:"#1e2330",borderRadius:12,border:"1px solid #2a2f3d",padding:14,marginBottom:12}}>
      ...
      <div style={{display:"flex",gap:4}}>
        {days.map((d,i)=>{ const pct=d.total>0?d.done/d.total:0, isToday=i===todayDayIdx(); return (
...
export default function App() {
  ...
  // Backup data object
  const backupData = { categories, selectedCat, weeklyTasks, customTasks, logs, diaries, goalHours, exportedAt: new Date().toISOString() };

  const handleRestore = (parsed) => {
    if(parsed.categories)   setCategories(parsed.categories);
    if(parsed.selectedCat)  setSelectedCat(parsed.selectedCat);
  ...
  const handleStop=()=>{
    ...
    if(total>=10){ const cat=categories.find(c=>c.id===selectedCat)||categories[0]; setLogs(p=>[{id:Date.now(),date:todayStr(),label:cat.name,catId:cat.id,duration:total,mode},...p]); }
  ...
  const addTask=dayIdx=>{ if(!newTaskInput.trim()) return; setCustomTasks(p=>({...p,[dayIdx]:[...p[dayIdx],{id:Date.now(),label:newTaskInput.trim(),done:false}]})); setNewTaskInput(""); setAddingDay(null); };

  const handleDrop=useCallback((task,fromDay,toDay)=>{
  ...
  const { getDragProps, getDropZoneProps } = useTouchDrag(handleDrop);
      ...
      <div>
        <div style={{...S.card,background:"#161920",borderColor:"rgba(79,158,255,0.2)"}}>
          ...
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div><div style={{fontSize:10,color:"#6b7a99"}}>今日の合計</div><div style={{fontSize:20,fontWeight:800,color:catColor,fontFamily:"monospace"}}>{fmtTime(todayTotal)}</div></div>
            <div style={{display:"flex",gap:6}}>
              <button onClick={()=>setShowDiaryList(true)} style={{background:"rgba(255,255,255,0.04)",border:"1px solid #2a2f3d",borderRadius:8,padding:"7px 10px",cursor:"pointer",color:"#6b7a99",fontSize:11,fontWeight:600}}>📔 一覧</button>
              <button onClick={()=>setDiaryModal(todayStr())} style={{background:diaries[todayStr()]?"rgba(251,191,36,0.15)":"rgba(255,255,255,0.04)",border:`1px solid ${diaries[todayStr()]?"#fbbf24":"#2a2f3d"}`,borderRadius:8,padding:"7px 10px",cursor:"pointer",color:diaries[todayStr()]?"#fbbf24":"#6b7a99",fontSize:11,fontWeight:600}}>
                {diaries[todayStr()]?"📔 今日の日記":"✏️ 日記"}
              </button>
            </div>
          </div>
        </div>

        <div style={{fontSize:11,color:"#6b7a99",marginBottom:8,display:"flex",alignItems:"center",gap:6}}>
          <span style={{fontSize:12,fontWeight:800,color:"#e8ecf4"}}>📋 今週のタスク</span>
          <span style={{fontSize:10}}>— ⠿ を長押しで移動</span>
        </div>
        ...
        <div style={{display:"grid",gridTemplateColumns:mobile?"1fr":"repeat(3,1fr)",gap:8}}>
          {DAYS_LABEL.map((day,i)=>{
            ...
            const dzProps=getDropZoneProps(i);
            return (
              <div key={i} {...dzProps} style={{...S.card,padding:10,borderColor:isToday?catColor:"#2a2f3d",background:isToday?`rgba(${hexRgb(catColor)},0.06)`:"#1e2330",transition:"outline 0.15s,background 0.15s"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    ...
                    <button onClick={()=>setDiaryModal(dayDate)} title="日記" style={{background:"none",border:"none",cursor:"pointer",padding:0}}>
                      <div style={{width:7,height:7,borderRadius:"50%",background:hasDiary?"#fbbf24":"#2a2f3d",boxShadow:hasDiary?"0 0 6px #fbbf2488":"none",transition:"all 0.2s"}}/>
                    </button>
                  </div>
                ...
                {all.map(t=>{
                  const dp=getDragProps(t,i);
                  return (
                    <div key={t.id} style={{display:"flex",alignItems:"center",gap:6,padding:"6px 0",borderBottom:"1px solid #2a2f3d",opacity:t.done?0.4:1}}>
                      <span {...dp} style={{color:"#3d4560",fontSize:14,flexShrink:0,cursor:"grab",userSelect:"none",WebkitUserSelect:"none",touchAction:"none",padding:"2px 3px"}} title="長押しで移動">⠿</span>
                      <div onClick={()=>toggleTask(i,t.id,t.weekly)} style={{width:14,height:14,borderRadius:4,flexShrink:0,border:`2px solid ${t.done?"#34d399":"#3d4560"}`,background:t.done?"#34d399":"transparent",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                        {t.done&&<span style={{color:"#fff",fontSize:9}}>✓</span>}
                      </div>
                      <span style={{fontSize:11,flex:1,lineHeight:1.3,minWidth:0,wordBreak:"break-all"}}>{t.label}</span>
                ...
                {addingDay===i?(
                  <div style={{display:"flex",gap:4,marginTop:6}}>
                    <input autoFocus style={{...S.input,fontSize:11,padding:"5px 8px"}} placeholder="タスク名" value={newTaskInput}
                      onChange={e=>setNewTaskInput(e.target.value)}
                      onKeyDown={e=>{ if(e.key==="Enter")addTask(i); if(e.key==="Escape"){setAddingDay(null);setNewTaskInput("");} }}/>
                    <button style={{...S.btn(),padding:"5px 8px",fontSize:11}} onClick={()=>addTask(i)}>✓</button>
                    <button style={{...S.btn("#2a2f3d"),padding:"5px 8px",fontSize:11}} onClick={()=>{setAddingDay(null);setNewTaskInput("");}}>✕</button>
                  </div>
                ):(
                  <button onClick={()=>{setAddingDay(i);setNewTaskInput("");}} style={{width:"100%",background:"none",border:"1px dashed #2a2f3d",borderRadius:6,padding:"5px 0",color:"#3d4560",cursor:"pointer",fontSize:16,marginTop:6}}>+</button>
                )}
              </div>
            );
          })}
  ...
  const TimerTab=()=>(
    <div style={{maxWidth:400,margin:"0 auto"}}>
      ...
      {pomoDone&&<div style={{background:"rgba(52,211,153,0.12)",border:"1px solid #34d399",borderRadius:10,padding:10,textAlign:"center",marginBottom:12,fontSize:12,color:"#34d399",fontWeight:700}}>🎉 {pomoDuration}分完了！計測は継続中</div>}
      {running&&<div style={{textAlign:"center",marginBottom:8,fontSize:11,color:"#6b7a99"}}>カテゴリー: <span style={{color:catColor,fontWeight:700}}>{categories.find(c=>c.id===selectedCat)?.name}</span></div>}
      ...
      {elapsed>0&&<div style={{textAlign:"center",marginTop:12,color:"#6b7a99",fontSize:12}}>経過: <span style={{color:"#e8ecf4",fontWeight:700,fontFamily:"monospace"}}>{fmtTime(elapsed)}</span></div>}
    </div>
  );
    ...
    return (
      <div>
        ...
        <div style={{...S.card,display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:12}}>
          ...
          {editGoal
            ?<div style={{display:"flex",gap:6,alignItems:"center"}}><input style={{...S.input,width:54,textAlign:"center"}} type="number" min="1" value={gInput} onChange={e=>setGInput(e.target.value)}/><span style={{color:"#6b7a99",fontSize:12}}>時間</span><button style={S.btn()} onClick={()=>{setGoalHours(Math.max(1,Number(gInput)));setEditGoal(false);}}>✓</button></div>
            :<div style={{display:"flex",alignItems:"center",gap:8}}><span style={{fontFamily:"monospace",fontSize:14,fontWeight:800,color:"#34d399"}}>{goalHours}h</span><button style={{...S.btn("#2a2f3d"),padding:"5px 10px",fontSize:11}} onClick={()=>setEditGoal(true)}>変更</button></div>
          }
        </div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
            ...
            <div key={date} style={{marginBottom:16}}>
              ...
              {dl.map(l=>{ const cat=categories.find(c=>c.id===l.catId); return (
                <div key={l.id} style={{...S.card,display:"flex",alignItems:"center",gap:8,padding:"9px 12px",marginBottom:6}}>
                  ...
                  <div style={{flex:1}}><div style={{fontSize:12,fontWeight:700}}>{l.label}</div><div style={{fontSize:10,color:"#6b7a99",marginTop:1}}>{l.mode==="pomodoro"?"🍅":"⏱"}{cat&&<span style={{marginLeft:4,color:cat.color}}>#{cat.name}</span>}</div></div>
  ...
  return (
    <div style={S.app}>
      {/* Full-screen timer lock when running */}
      {running&&tab!=="timer"&&(
        <div style={{position:"fixed",inset:0,zIndex:90,background:"#0d0f14",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center"}}>
          <div style={{fontSize:11,color:"#6b7a99",marginBottom:8,letterSpacing:2,textTransform:"uppercase"}}>計測中</div>
          <RingTimer elapsed={elapsed} total={mode==="pomodoro"?pomoDuration*60:0} running={true} color={catColor}/>
          <div style={{marginTop:8,fontSize:11,color:"#6b7a99"}}>カテゴリー: <span style={{color:catColor,fontWeight:700}}>{categories.find(c=>c.id===selectedCat)?.name}</span></div>
      ...
      <div style={S.header}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <button onClick={()=>setShowBackup(true)} style={{background:"none",border:"none",color:"#3d4560",cursor:"pointer",fontSize:18,lineHeight:1,padding:2}} title="バックアップ">💾</button>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
      ...
      {showBackup&&<BackupModal data={backupData} onRestore={handleRestore} onClose={()=>setShowBackup(false)}/>}
    </div>
  );
}
