            ...
            return (
              <div key={i} style={{...S.card,padding:10,borderColor:isToday?catColor:"#2a2f3d",background:isToday?`rgba(${hexRgb(catColor)},0.06)`:"#1e2330"}}>
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                  ...
                  {(()=>{
                    const d = diaries[fmtDate(getDayDate(i))];
                    const hasDiary = getDiaryText(d)?.trim();
                    const dc = DIARY_COLORS.find(c=>c.id===getDiaryColor(d));
                    return (
                      <button onClick={()=>setDiaryModal(fmtDate(getDayDate(i)))} style={{
      ...
      <div style={S.header}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            ...
            <div style={{fontSize:10,color:"#6b7a99"}}>タスク & 時間管理 <span style={{color:"#3d4560",marginLeft:4}}>v2.4.0</span></div>
          </div>
