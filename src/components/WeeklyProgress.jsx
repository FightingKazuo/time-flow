...
export default function WeeklyProgress({ weeklyTasks, customTasks, logs, diaries, goalHours, onSelectDay, selectedDay, studyCatId, theme }) {
  ...
  const days = DAYS_LABEL.map((_,i)=>{
    const wt=weeklyTasks[i]||[], ct=customTasks[i]||[], all=[...wt,...ct];
    return {day:DAYS_LABEL[i], total:all.length, done:all.filter(t=>t.done).length};
  });
  ...
  for(let i=0;i<7;i++){
    ...
    if(getDiaryText(diaries[fmtDate(d)])?.trim()) diaryCount++;
  }
  ...
  const Ring = ({pct, color, label, sub: subLabel, achieved}) => {
    ...
    return (
      <div style={{textAlign:"center"}}>
        ...
        <div style={{fontSize:BASE_FONT-4,fontWeight:700,color:text,marginTop:2}}>{label}{achieved&&<span style={{marginLeft:2}}>🎯</span>}</div>
        <div style={{fontSize:BASE_FONT-5,color:sub}}>{subLabel}</div>
      </div>
    );
  };

  return (
    <div style={{background:card,borderRadius:12,border:`1px solid ${border}`,padding:14,marginBottom:12,boxShadow:shadow}}>
      {goalReached&&(
      ...
      <div style={{fontSize:BASE_FONT-2,fontWeight:800,marginBottom:12,color:sub}}>今週の進捗</div>

      <div style={{display:"flex",justifyContent:"space-around",alignItems:"flex-start",marginBottom:14}}>
        <Ring pct={taskPct}  color={accent}    label="タスク"   sub={`${doneT}/${totalT}`}/>
        ...
        <div style={{textAlign:"center",paddingTop:6}}>
          ...
          <div style={{fontSize:BASE_FONT-5,fontWeight:700,color:text,marginTop:3}}>今日合計</div>
          <div style={{fontSize:BASE_FONT-5,color:accent,marginTop:2}}>{fmtHM(studyTodayTotal)||"0m"} 勉強</div>
        </div>
      </div>
      ...
      <div style={{display:"flex",gap:4,marginBottom:10}}>
        {days.map((d,i)=>{
          const pct=d.total>0?d.done/d.total:0, isSel=i===selectedDay, isToday=i===todayDayIdx();
          return (
            <div key={i} onClick={()=>onSelectDay(i)} style={{flex:1,textAlign:"center",cursor:"pointer"}}>
              <div style={{height:40,background:card2,borderRadius:4,overflow:"hidden",display:"flex",flexDirection:"column-reverse",border:isSel?`1.5px solid ${accent}`:"1.5px solid transparent",transition:"border 0.2s"}}>
                <div style={{height:`${pct*100}%`,background:isToday?accent:border,transition:"height 0.5s",borderRadius:4}}/>
              </div>
              <div style={{fontSize:BASE_FONT-5,color:isSel?accent:isToday?sub:muted,marginTop:3,fontWeight:isSel||isToday?700:400}}>{d.day}</div>
            </div>
          );
        })}
      </div>
      ...
      {/* 選択日詳細 */}
      <div style={{background:card2,borderRadius:8,padding:10,border:`1px solid ${border}`}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
          <span style={{fontSize:BASE_FONT-2,fontWeight:700,color:accent}}>{DAYS_LABEL[selectedDay]} {dayDateStr(selectedDay)} の進捗</span>
          {getDiaryText(diaries[selDate])?.trim()
            ...
            :<span style={{fontSize:BASE_FONT-4,color:muted,background:card,border:`1px solid ${border}`,borderRadius:6,padding:"2px 8px"}}>日記なし</span>
          }
        </div>
        <div style={{display:"flex",gap:16}}>
          <div style={{textAlign:"center"}}><div style={{fontSize:18,fontWeight:800,color:accent}}>{selTaskPct}%</div><div style={{fontSize:BASE_FONT-5,color:sub}}>タスク {selD.done}/{selD.total}</div></div>
          <div style={{textAlign:"center"}}><div style={{fontSize:18,fontWeight:800,color:"#34d399"}}>{fmtHM(selStudyTotal)||"0m"}</div><div style={{fontSize:BASE_FONT-5,color:sub}}>勉強時間</div></div>
          <div style={{textAlign:"center"}}><div style={{fontSize:18,fontWeight:800,color:sub}}>{fmtHM(selTotal)||"0m"}</div><div style={{fontSize:BASE_FONT-5,color:sub}}>合計時間</div></div>
        </div>
      </div>
    </div>
  );
