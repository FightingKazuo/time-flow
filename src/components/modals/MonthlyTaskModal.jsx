...
export default function MonthlyTaskModal({ tasks, onSave, onClose }) {
    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:500}} onClick={()=>{onSave(items);onClose();}}>
      <div style={{background:"#1e2330",borderRadius:"20px 20px 0 0",border:"1px solid #2a2f3d",padding:20,width:"100%",maxWidth:480,maxHeight:"88vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
          <div style={{fontSize:BASE_FONT+2,fontWeight:800}}>🗓 マンスリータスク</div>
        ...
        <div style={{background:"#161920",borderRadius:10,padding:12,marginBottom:12,border:"1px solid #2a2f3d"}}>
          <div style={{display:"flex",gap:6,marginBottom:8}}>
            <button onClick={()=>setMode("weekday")} style={{flex:1,padding:"7px 0",borderRadius:8,border:`1px solid ${mode==="weekday"?"#4f9eff":"#2a2f3d"}`,background:mode==="weekday"?"rgba(79,158,255,0.12)":"transparent",color:mode==="weekday"?"#4f9eff":"#6b7a99",cursor:"pointer",fontWeight:700,fontSize:BASE_FONT-3}}>第○曜日</button>
            <button onClick={()=>setMode("date")} style={{flex:1,padding:"7px 0",borderRadius:8,border:`1px solid ${mode==="date"?"#4f9eff":"#2a2f3d"}`,background:mode==="date"?"rgba(79,158,255,0.12)":"transparent",color:mode==="date"?"#4f9eff":"#6b7a99",cursor:"pointer",fontWeight:700,fontSize:BASE_FONT-3}}>毎月○日</button>
          </div>
          ...
          {mode==="weekday"?(
            ...
            <div style={{display:"flex",gap:6,marginBottom:8,alignItems:"center"}}>
              <span style={{color:"#6b7a99",fontSize:BASE_FONT-2}}>毎月</span>
              ...
              <span style={{color:"#6b7a99",fontSize:BASE_FONT-2}}>日</span>
            </div>
          )}
        ...
        <div style={{overflowY:"auto",flex:1}}>
          {items.length===0&&<div style={{textAlign:"center",color:"#6b7a99",padding:20,fontSize:BASE_FONT-2}}>マンスリータスクなし</div>}
          {items.map(t=>(
            <div key={t.id} style={{display:"flex",alignItems:"center",gap:8,padding:"10px 0",borderBottom:"1px solid #2a2f3d"}}>
              <div style={{flex:1}}>
                ...
                <div style={{fontSize:11,color:"#4f9eff",marginTop:2}}>{describeTask(t)}</div>
              </div>
              <button onClick={()=>setItems(p=>p.filter(x=>x.id!==t.id))} style={{background:"none",border:"none",color:"#3d4560",cursor:"pointer",fontSize:16}}>✕</button>
            </div>
          ))}
        </div>
