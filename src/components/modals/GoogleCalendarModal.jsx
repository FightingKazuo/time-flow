...
export async function autoSyncGoogleCalendar(existingEvents, onMerge) {
  ...
  if(!token || Date.now() > expiry) return; // トークンなし or 期限切れ
  ...
  try {
    ...
    if(res.status === 401) { LS.set(TOKEN_KEY, null); return; } // 期限切れ
    if(!res.ok) return;
      ...
      <div style={{background:"#1e2330",borderRadius:"20px 20px 0 0",border:"1px solid #2a2f3d",padding:24,width:"100%",maxWidth:480,maxHeight:"88vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <div>
            ...
            <div style={{fontSize:11,color:"#6b7a99"}}>過去1週間〜30日後 · 重複は自動スキップ</div>
          </div>
          {status==="authed"&&<button onClick={()=>{setStatus("idle");setEvents([]);setSelected(new Set());LS.set(TOKEN_KEY,null);}} style={{background:"none",border:"1px solid #3d4560",borderRadius:8,padding:"5px 10px",color:"#6b7a99",cursor:"pointer",fontSize:BASE_FONT-3}}>ログアウト</button>}
        </div>
        ...
        {status==="idle"&&<div style={{textAlign:"center",padding:"20px 0",flex:1}}><div style={{fontSize:48,marginBottom:12}}>📅</div><div style={{fontSize:BASE_FONT-1,color:"#6b7a99",marginBottom:24,lineHeight:1.6}}>Googleアカウントでサインインして<br/>カレンダーの予定を取得します<br/><span style={{fontSize:11,color:"#3d4560"}}>次回以降は自動で同期されます</span></div><button style={bS("#4285f4")} onClick={signIn}>Googleでサインイン</button></div>}
        {status==="loading"&&<div style={{textAlign:"center",padding:"40px 0",color:"#6b7a99",fontSize:BASE_FONT,flex:1}}><div style={{fontSize:32,marginBottom:12}}>⏳</div>認証中・取得中...</div>}
        {status==="authed"&&<>
          <div style={{fontSize:BASE_FONT-2,color:"#6b7a99",marginBottom:8}}>{events.length}件の予定 · {selected.size}件選択中</div>
          <div style={{overflowY:"auto",flex:1,marginBottom:12}}>
            {events.length===0&&<div style={{textAlign:"center",color:"#6b7a99",padding:20}}>この期間に予定が見つかりません</div>}
            {events.map(e=>(
              <div key={e.id} onClick={()=>toggleSelect(e.id)} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"10px 0",borderBottom:"1px solid #2a2f3d",cursor:"pointer"}}>
                <div style={{width:18,height:18,borderRadius:5,flexShrink:0,marginTop:2,border:`2px solid ${selected.has(e.id)?"#4285f4":"#3d4560"}`,background:selected.has(e.id)?"#4285f4":"transparent",display:"flex",alignItems:"center",justifyContent:"center"}}>
                  {selected.has(e.id)&&<span style={{color:"#fff",fontSize:11}}>✓</span>}
                </div>
                <div style={{flex:1}}>
                  <div style={{fontSize:BASE_FONT-1,fontWeight:600}}>{e.title}</div>
                  <div style={{fontSize:11,color:"#4f9eff",marginTop:2}}>{fmt(e.date)}{e.time&&<span style={{marginLeft:6}}>⏰{e.time}</span>}</div>
                  {e.note&&<div style={{fontSize:10,color:"#6b7a99",marginTop:2}}>{e.note.slice(0,60)}</div>}
                </div>
              </div>
            ))}
          </div>
