...
export default function RingTimer({ elapsed, total, running, color, theme }) {
  ...
  return (
    <svg width="240" height="240" viewBox="0 0 220 220">
      ...
      {dh&&<text x="110" y="95" textAnchor="middle" fill={theme?.sub||"#6b7a99"} fontSize="14" fontFamily="monospace">{dh}h</text>}
      <text x="110" y={dh?"122":"118"} textAnchor="middle" fill={textColor} fontSize={dh?"32":"38"} fontWeight="800" fontFamily="monospace">{dm}:{ds}</text>
      <text x="110" y="142" textAnchor="middle" fill={running?rc:mutedColor} fontSize="12" fontFamily="monospace" fontWeight="600">{running?"● REC":"⏸ PAUSE"}</text>
    </svg>
  );
}
