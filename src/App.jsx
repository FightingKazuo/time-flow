    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:mobile?"1fr":"repeat(3,1fr)",gap:8}}>
        {DAYS_LABEL.map((day, i) => {
          ...
          return (
            <div key={i} style={{...S.card,padding:10,borderColor:isToday?T.accent:T.border,background:isToday?`rgba(${hexRgb(T.accent)},0.04)`:T.card,boxShadow:isToday?`0 0 0 1.5px ${T.accent}22, ${T.shadow}`:T.shadow}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                <div style={{display:"flex",alignItems:"center",gap:5}}>
                  ...
                  <button onClick={()=>setDiaryModal(dayDate)} style={{background:hasDiary?"rgba(251,191,36,0.15)":T.card2,border:`1.5px solid ${hasDiary?"#fbbf24":T.border}`,borderRadius:8,padding:"3px 8px",cursor:"pointer",fontSize:11,fontWeight:800,color:hasDiary?"#fbbf24":T.sub,lineHeight:"18px"}}>
                    {hasDiary ? "📔" : "＋日記"}
                  </button>
                </div>
...
// ─── Timer Tab ────────────────────────────────────────────────────────────────
function TimerTab({ T, S, categories, selectedCat, setSelectedCat, studyCatId, mode, setMode,
  pomoDuration, setPomoDuration, elapsed, running, timerStart, timerPause, handleStop,
  setShowCatMgr, setTab, catColor, todayStudyTotal, pomoDone }) {
  ...
  return (
    <div style={running ? {position:"fixed",inset:0,zIndex:88,background:T.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:20} : {maxWidth:440,margin:"0 auto"}}>
      ...
      {mode === "timer" && (
        <div style={{textAlign:"center",marginBottom:10}}>
          <div style={{fontSize:BASE_FONT-2,color:T.sub,marginBottom:2}}>今日の{categories.find(c=>c.id===studyCatId)?.name||"勉強"}時間</div>
...
// ─── Diary Screen ─────────────────────────────────────────────────────────────
function DiaryScreen({ T, S, diaryModal, diaries, saveDiary, setDiaryModal }) {
  ...
  const entries  = Object.entries(diaries).filter(([,d])=>getDiaryText(d)?.trim()).sort((a,b)=>b[0].localeCompare(a[0]));
    ...
    <div style={{position:"fixed",inset:0,zIndex:300,background:T.bg,display:"flex",flexDirection:"column",overflowY:"auto"}}>
      ...
      <div style={{padding:16,display:"flex",flexDirection:"column",gap:16}}>
        ...
        <div style={{textAlign:"center",padding:16}}>
          <div style={{fontSize:12,color:`${T.sub}88`}}>TimeFlow {APP_VERSION}</div>
        </div>
      </div>
    </div>
  );
...
// ─── Log Tab ──────────────────────────────────────────────────────────────────
function LogTab({ T, S, logs, diaries, categories, studyCatId, weeklyTasks, customTasks,
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        ...
        <button onClick={()=>setShowDiaryList(true)} style={{padding:"10px 14px",borderRadius:10,border:"1px solid #fbbf2440",background:"rgba(251,191,36,0.08)",color:"#fbbf24",fontWeight:700,cursor:"pointer",fontSize:BASE_FONT-2,boxShadow:T.shadow}}>📔</button>
        <button onClick={()=>setShowDiaryAnalysis(true)} style={{padding:"10px 14px",borderRadius:10,border:`1px solid ${T.accent}44`,background:"rgba(79,142,247,0.08)",color:T.accent,fontWeight:700,cursor:"pointer",fontSize:BASE_FONT-2,boxShadow:T.shadow}}>✨</button>
      </div>
...
export default function App() {
  ...
  const { elapsed, running, start: timerStart, pause: timerPause, stop: timerStop } = useTimer({
    ...
    onAutoStop: (info) => { setAutoStopInfo(info); setTab("log"); notify("⏱ タイマー自動停止","2時間を超えたため自動停止しました。"); },
  });
  ...
  const saveWeeklyTemplates = (tpls) => {
    ...
    setWeeklyTasks(prev => {
      ...
      DAYS_LABEL.forEach((_,i)=>{ next[i]=next[i].map(t=>{ const old=(prev[i]||[]).find(o=>o.id===t.id); return old?{...t,done:old.done}:t; }); });
      return next;
    });
  ...
  const toggleTask = (dayIdx,id,weekly) => {
    if(weekly) {
      ...
      setCustomTasks(p=>{
        ...
        if(task?.fromLongTerm) setLongTermTasks(prev=>prev.map(lt=>lt.id===task.fromLongTerm?{...lt,done:task.done,doneAt:task.done?todayStr():null}:lt));
        return next;
      });
    }
  };
    ...
    <div style={S.app}>
      ...
      {autoStopInfo && (
        <div style={{position:"fixed",top:0,left:0,right:0,zIndex:600,background:"#fb923c",padding:"10px 16px",display:"flex",alignItems:"center",gap:10}}>
          ...
          <button onClick={()=>{ const cat=categories.find(c=>c.id===(autoStopInfo.savedState?.catId||selectedCat))||categories[0]; setEditingLog({id:Date.now(),date:autoStopInfo.savedState?.date||todayStr(),label:cat.name,catId:cat.id,duration:autoStopInfo.duration,mode:autoStopInfo.savedState?.mode||mode,startHour:autoStopInfo.savedState?.startHour||null,_isAutoStop:true}); setAutoStopInfo(null); setTab("log"); }} style={{background:"#fff",color:"#fb923c",border:"none",borderRadius:8,padding:"8px 14px",fontWeight:800,cursor:"pointer",fontSize:BASE_FONT-2,flexShrink:0}}>
            時間を調整して記録
          </button>
      ...
      <div style={S.header}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div>
            ...
            <div style={{fontSize:10,color:T.sub}}>タスク & 時間管理 <span style={{color:`${T.sub}88`,marginLeft:4}}>{APP_VERSION}</span></div>
          </div>
      ...
      {editingLog      && <EditLogModal log={editingLog} categories={categories} onSave={u=>{ if(u._isAutoStop){const{_isAutoStop,...clean}=u;setLogs(p=>[{...clean,id:Date.now()},...p]);}else{setLogs(p=>p.map(l=>l.id===u.id?u:l));} setEditingLog(null); }} onClose={()=>setEditingLog(null)} theme={T}/>}
      ...
      {showBackup      && <BackupModal
        data={{categories,selectedCat,studyCatId,weeklyTemplates,weeklyTasks,customTasks,longTermTasks,logs,diaries,goalHours,weekHistory,exportedAt:new Date().toISOString(),appVersion:APP_VERSION}}
        onRestore={p=>{ if(p.categories)setCategories(p.categories); if(p.selectedCat)setSelectedCat(p.selectedCat); if(p.studyCatId)setStudyCatId(p.studyCatId); if(p.weeklyTemplates)setWeeklyTemplates(p.weeklyTemplates); if(p.weeklyTasks)setWeeklyTasks(p.weeklyTasks); if(p.customTasks)setCustomTasks(p.customTasks); if(p.longTermTasks)setLongTermTasks(p.longTermTasks); if(p.logs)setLogs(p.logs); if(p.diaries)setDiaries(p.diaries); if(p.goalHours)setGoalHours(p.goalHours); if(p.weekHistory)setWeekHistory(p.weekHistory); }}
